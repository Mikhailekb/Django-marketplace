...
// }, 100);
        ...
        for (let i = 0; i < sortLinks.length; i++) {
            sortLinks[i].addEventListener('click', clickToSortLink);
        }

        function clickToSortLink(event) {
        ...
        for (let i = 0; i < paginationLinks.length; i++) {
            paginationLinks[i].addEventListener('click', clickToPaginationLink);
        }

        function clickToPaginationLink(event) {
            event.preventDefault();
            let currentUrl_obj = new URL(window.location.href);
            let linkUrl = this.href;
